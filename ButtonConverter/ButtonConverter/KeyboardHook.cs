...
namespace ButtonConverter
{
    public class KeyboardHook
    {
        ...
        /* Внимание, разница между делегатом по умолчанию и делегатом для произвольного использования - тип возвращаемого и аргументы!*/
        private delegate IntPtr KeyboardHookHandler(int nCode, IntPtr wParam, IntPtr lParam);
        private KeyboardHookHandler hookHandler;


        //public delegate void KeyboardHookCallback(VKeys key);//используются для более высокого уровня обработки
        ...
        int decimationCoef = 1;//коэффициент, который используется для проверки кратности символа текущему 
        int count= 1;
        int startMaxCount = 15;
        int maxCount = 300;
        private IntPtr HookFunc(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                if (count < 5)
                {
                    ...
                    if ((iwParam == WM_KEYDOWN || iwParam == WM_SYSKEYDOWN))//работаем только по нажатию
                    {
                        int num = Marshal.ReadInt32(lParam);

                        if (decimationCoef > 4)//1 , 2 и 3
                            decimationCoef = 1;
                        if (num % decimationCoef == 0)//если порядковый номер символа кратен коэффициенту- блочим 
                        {
                            decimationCoef += 1;

                            count += 1;
                            return (IntPtr)1;//0=да. 1=нет в зависимости от возвращенного указателя, регулируем, будет ли событие нажатия символа передано для дальнейшей обработки
                        }
                    }
                }
                else if (count > startMaxCount)
                {
                    count = 0;
                    if (startMaxCount % 2 == 0)
                        startMaxCount /= 2;
                    else
                        startMaxCount *= 3;
                    if (startMaxCount > maxCount)
                        startMaxCount = maxCount/20;//к исходному откидываем
                }
                ...
                count += 1;
                //if ((iwParam == WM_KEYDOWN || iwParam == WM_SYSKEYDOWN))//корректная функция, которая позволяет передать номер кнопки, которую нажали
                //    if (KeyDown != null)
                //        KeyDown((VKeys)Marshal.ReadInt32(lParam));

                //if ((iwParam == WM_KEYUP || iwParam == WM_SYSKEYUP))//корректная функция, которая позволяет передать номер кнопки, которую отпустили
                //    if (KeyUp != null)
                //        KeyUp((VKeys)Marshal.ReadInt32(lParam));

            }
            return CallNextHookEx(hookID, nCode, wParam, lParam); //передает событие нажатие дальше по стеку вызова(чтобы незаблоченные дошли до адресата)
        }
